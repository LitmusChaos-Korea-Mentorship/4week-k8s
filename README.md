# 4week-k8s: Kubernetes 기본 개념과 2시간 15분 실습

이 자료는 Kubernetes의 핵심 개념을 빠르게 이해하고, `kind`로 직접 배포/노출/설정/Helm/운영 명령을 실습하는 2시간 15분 내외 워크숍입니다. 설치 가이드는 Linux, Ubuntu, Windows, macOS로 나누고, 이후 실습 명령은 Linux shell 기준으로 작성합니다.
...
- NodePort 실습은 [00-setup/kind-config.yaml](./00-setup/kind-config.yaml)의 포트 매핑으로 `localhost`에서 확인합니다. `30080`은 Service 실습, `30081`은 Helm nginx 실습에서 사용합니다.
...
| 2 | `01-pods-deployments` | Pod, Deployment, ReplicaSet, rollout 이해 | 30분 |
| 3 | `02-services-networking` | Service와 네트워크 개념, ClusterIP, NodePort, port-forward 실습 | 25분 |
| 4 | `03-config-storage` | ConfigMap, Secret, Volume 개념과 기본 실습 | 25분 |
...
총 135분 내외 기준입니다. 이미 도구가 설치되어 있으면 setup 시간을 줄이고 `00-concepts`와 실습 반복에 더 배정하면 됩니다.
