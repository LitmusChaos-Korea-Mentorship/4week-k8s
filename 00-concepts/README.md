...
이 Pod IP에 직접 접근하는 방식은 좋지 않습니다. Pod가 삭제되고 새로 만들어지면 IP가 바뀌기 때문입니다. Service는 `app=web` 같은 label selector로 Pod들을 찾고, 그 앞에 고정된 이름과 IP를 제공합니다.

```text
Client -> Service/web-clusterip -> app=web label을 가진 Pod들
```
...
- `metadata.name`: Service 이름입니다. 클러스터 안에서는 이 이름으로 DNS 접근이 가능합니다.
- `spec.type`: Service 노출 방식을 정합니다.
- `spec.selector`: 트래픽을 보낼 Pod를 label로 찾습니다.
- `ports.port`: Service가 받을 포트입니다.
- `ports.targetPort`: 실제 Pod 컨테이너로 전달할 포트입니다.

예를 들어 같은 namespace 안의 다른 Pod에서는 다음처럼 접근할 수 있습니다.
...
`port-forward`는 Service 타입은 아닙니다. 로컬 PC의 포트를 Kubernetes 리소스에 임시로 연결하는 디버깅 방법입니다.
...
- Service selector와 Pod label이 맞지 않으면 Service는 만들어져도 트래픽을 보낼 Pod를 찾지 못합니다.
